...
dict that ``compressai.models.MambaIC.from_state_dict`` can load directly.
Optionally reports forward-pass sanity numbers (PSNR / bpp) on a synthetic
input.

The upstream-vs-compressai key differences (the ``nn.Sequential`` wrapper
around each ``SWAtten``, top-level ``atten_*`` / ``anchor_atten_*`` /
``context_*`` / ``cc_*`` / ``lrp_*`` / ``gaussian_conditional`` -> the
``latent_codec.*`` namespace, the duplicated ``context_vss_{1..N}``
attributes that alias ``context_vss.{i}``, and the WMSA / Block layout
differences) are all handled inside ``MambaIC._migrate_state_dict``; this
...
from compressai.models import MambaIC
