...
LIC_TCM repo, https://github.com/jmliu206/LIC_TCM), and writes a state dict
...
around each ``SWAtten``, ``atten_mean`` -> ``latent_codec.mean_support_transforms``,
WMSA layout, layer-norm names, etc.) are all handled inside
``TCM._migrate_state_dict``; this script is a thin CLI around it.

Example::

    python examples/convert_tcm_checkpoint.py \\
        --src candidate/LIC_TCM/0.013.pth..tar \\
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to the upstream LIC-TCM checkpoint (e.g. 0.013.pth..tar).",
    )
    parser.add_argument(
        "--dst",
        type=Path,
