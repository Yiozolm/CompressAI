...
class DCAE(CompressionModel):
    ...
    .. code-block:: text
        ...
        latent_codec = HyperpriorLatentCodec(
            h_a=h_a,
            h_s=DualHyperSynthesis(h_mean_s, h_scale_s),  # original h_z_s2/h_z_s1 swapped to means/scales
            latent_codec={
                "z": EntropyBottleneckLatentCodec(EntropyBottleneck(N), quantizer="noise"),
                "y": ChannelGroupsLatentCodec(side_in_context=True, ...) wired
                     inline with per-slice DictionaryMeanScaleContextHead +
                     LRPGaussianLatentCodec(mean_support_trail_channels=...).
            },
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        def _make_hyper_synthesis() -> nn.Sequential:
        ...
        # DualHyperSynthesis(h_mean_s, h_scale_s) flips this to the
        # (means, scales) ordering shared with STF / TCM / CCA. The convert
        # script renames h_z_s2 -> h_s.h_mean_s and h_z_s1 -> h_s.h_scale_s.
        h_mean_s = _make_hyper_synthesis()
        h_scale_s = _make_hyper_synthesis()
        ...
        # Family 1 side_in_context wiring, inlined ELIC-style (mirrors STF /
        # TCM). channel_context covers y0..y(K-1); each head sees
