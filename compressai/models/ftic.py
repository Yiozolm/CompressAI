...
class WindowFrequencyModulation(nn.Module):
    ...
    def forward(self, input_tensor: Tensor, height: int, width: int) -> Tensor:
        ...
        output, pad_h, pad_w = pad_to_window_multiple(
...
class SwinFDWA(nn.Module):
    ...
    def forward(self, input_tensor: Tensor, spatial_size: Tuple[int, int]) -> Tensor:
        ...
        qkv = (
            self.qkv(output)
            .reshape(batch_size, length, 3, channels)
            .permute(2, 0, 1, 3)
        )
        ...
        output = output + self.frequency_modulation(
            self.ffn(self.norm2(output)), height, width
        )
        return output
...
class TCAEntropyModel(nn.Module):
    ...
    def forward(self, hyper: Tensor, y: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        hyper_features = self.hyper_trans(hyper).view(
...
def _infer_num_slices(state_dict: Dict[str, Tensor], M: int) -> int:
    start_token_channels = state_dict[
        "tca.tca.start_token_from_hyperprior.weight"
    ].size(0)
...
def _split_drop_paths(
    ...
    for depth in config:
        splits.append(
            tuple(float(value) for value in all_paths[offset : offset + depth])
        )
        offset += depth
    return tuple(splits)
...
class FrequencyAwareTransFormer(CompressionModel):
    ...
    def compress(self, x: Tensor) -> Dict[str, object]:
        ...
        for slice_index in range(self.num_slices):
            ...
            lrp = lrps[:, start:end]
            y_slice = y[:, start:end]
            y_hat_slice, slice_strings = self.gaussian_conditional.compress(
                y_slice, scale, mu
            )
    ...
    def decompress(
        ...
        for slice_index in range(self.num_slices):
            ...
            y_hat_slice = self.gaussian_conditional.decompress(
                y_strings[slice_index], scale, mu
            )
            y_hat_coded[:, start:end] = y_hat_slice
            lrp_coded[:, start:end] = lrp
