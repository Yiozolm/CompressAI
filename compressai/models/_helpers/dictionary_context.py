...
- :class:`DictionaryMeanScaleContextHead` — per-slice channel-context head
  ...
  ``side_in_context=True`` mode just like
  :class:`~compressai.models._helpers.channel_context.MeanScaleContextHead`.
- :func:`build_dictionary_mean_scale_head` — convenience factory matching
  :func:`~compressai.models._helpers.channel_context.build_mean_scale_head`'s
...
class DictionaryMeanScaleContextHead(nn.Module):
    ...
    consumed by :class:`~compressai.latent_codecs.LRPGaussianLatentCodec`
    with ``mean_support_trail_channels = support_ch + dict_output_ch`` to
    recover the upstream ``cat(support, y_hat)`` LRP input layout, enabling
    ...
    means), whereas :class:`DualHyperSynthesis` and
    :class:`ChannelGroupsLatentCodec` produce ``cat([latent_means,
    latent_scales, *prev_y_hat])`` (means before scales) under the
    containerized wiring used here. The ``cc_mean`` / ``cc_scale`` /
...
def build_dictionary_mean_scale_head(
    ...
    *,
    ...
    support_ch
        ...
        :class:`ChannelGroupsLatentCodec`. Under ``side_in_context=True``
        wiring this equals ``2 * M + slice_ch * support_count`` (the head
        does no internal split — the cross-attention treats it as a flat
        support tensor).
    shared_dictionary
    ...
    emit_mean_support
        ...
        Pair with
        :class:`~compressai.latent_codecs.LRPGaussianLatentCodec` and
