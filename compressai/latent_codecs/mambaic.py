...
class MambaICLatentCodec(LatentCodec):
    """Dedicated channel-slice + checkerboard codec for MambaIC.

    Like the MLIC family, MambaIC is a "Family-2" entropy model: it splits the
    latent into channel slices (ELIC-style) and codes each slice in a two-pass
    anchor / non-anchor checkerboard. It is therefore *structurally* close to
    MLIC's :class:`~compressai.latent_codecs.MultiContextCheckerboardLatentCodec`,
    but it is kept as a **dedicated** codec rather than reusing that leaf for
    two concrete reasons:

    1. **Separate mean / scale heads.** MLIC's leaf runs a single
       ``entropy_parameters`` head per pass and ``chunk``\\s the result into
       ``(scale, mean)``. MambaIC instead carries mean and scale down two
       independent SWAtten chains end-to-end: ``mean_support_transforms`` /
       ``cc_mean_transforms`` / ``context_mean_transforms`` for the mean and
       the matching ``*_scale_*`` chain for the scale, each fed a different
       support tensor (``cat([latent_means, ...])`` vs ``cat([latent_scales,
       ...])``). This does not fit the single-head-then-chunk contract.

    2. **Codec assembly spans two pattern layers.** The MLIC refactor puts the
       channel-slice loop (and channel context) in the outer
       :class:`~compressai.latent_codecs.ChannelGroupsLatentCodec` and the
       per-group anchor/non-anchor pass in the inner leaf. MambaIC's per-slice
       transforms and its VSS (``context_vss``) spatial context straddle that
       boundary, so the slice loop is kept here in one place.

    Consequently this class owns the whole entropy model: it loops over
    ``num_slices`` channel slices internally, and within each slice runs the
    anchor pass (zero spatial context) then the non-anchor pass (masked
    ``context_prediction`` + ``context_vss``), each producing means/scales via
    the separate mean/scale chains, finishing with a per-slice LRP residual.

    These per-slice transforms (SWAtten support/context heads, VSS context
    stages, masked context prediction, LRP) are MambaIC-specific and have no
    second consumer, so they live as constructor arguments rather than being
    generalised into a shared leaf (cf. design doc §10.12).
    """
    ...
    def _compress_step(
        ...
        *,
        ...
        quantized = self.gaussian_conditional.decompress(
            strings, indexes, means=means_half
        )
    ...
    def forward(
        ...
        for slice_index, y_slice in enumerate(y.chunk(self.num_slices, dim=1)):
            ...
            y_anchor_hat = _keep_anchor(
                quantize_ste(y_slice - means_anchor) + means_anchor
            )
    ...
    def decompress(
        ...
        **kwargs: Any,
        ...
        if len(strings) != self.num_slices:
            raise ValueError(
                "strings must contain one [anchor, nonanchor] pair per slice"
            )
        ...
        for slice_index, slice_strings in enumerate(strings):
            if len(slice_strings) != 2:
                raise ValueError(
                    "Each slice stream must contain anchor and non-anchor strings"
                )
