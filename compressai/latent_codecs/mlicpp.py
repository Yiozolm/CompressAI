...
class MLICPlusPlusLatentCodec(LatentCodec):
        ...
        self.global_inter_context = nn.ModuleList(
            [
                LinearGlobalInterContext(
                    dim=slice_ch * index,
                    out_dim=slice_ch * 2,
                    num_heads=_select_num_heads(slice_ch * index),
                )
                if index
                else None
                for index in range(slice_num)
    ...
    def _entropy_coder_state(self) -> Tuple[List[List[int]], List[int], List[int]]:
        return (
            self.gaussian_conditional.quantized_cdf.tolist(),
    ...
    def _compress_single(
        ...
        for index, y_slice in enumerate(y_slices):
            ...
            slice_nonanchor = compress_symbols(
                self.gaussian_conditional,
                slice_nonanchor,
                scales_nonanchor,
                means_nonanchor,
        ...
        cdf, cdf_lengths, offsets = self._entropy_coder_state()
    ...
    def _decompress_single(
        ...
        for index in range(self.slice_num):
            (
                scales_anchor,
                means_anchor,
                global_inter_ctx,
            ...
            slice_anchor = decompress_symbols(
                self.gaussian_conditional,
                scales_anchor,
                means_anchor,
                decoder,
            ...
            slice_nonanchor = decompress_symbols(
                self.gaussian_conditional,
                scales_nonanchor,
                means_nonanchor,
                decoder,
