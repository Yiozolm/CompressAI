...
This is the only path currently used (see the package docstring in
``__init__``). State_dict layout (``qkv``, ``proj``, ``rpb``) matches the CUDA
build so weights are interchangeable should a fast path be wired later.
"""
...
class NeighborhoodAttentionTorch(nn.Module):
    ...
    def forward(self, x):
        ...
        q, x = x[:, :, :, :C], x[:, :, :, C:]
        q = (
            q.reshape(B, N, self.num_heads, C // self.num_heads, 1).transpose(3, 4)
            * self.scale
        )
...
def _warn_torch_fallback():
    warnings.warn(
        "Using the pure-PyTorch Neighborhood Attention fallback. Install the "
        "optional `natten` package (Linux+CUDA only) for a future CUDA fast "
        "path; the pure-PyTorch path is correct but slower.",
        stacklevel=3,
    )
