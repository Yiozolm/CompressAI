...
The underlying ``NeighborhoodAttention`` currently uses a vendored pure-PyTorch
reference (see ``compressai.layers.attn.natten``); a future revision can wire a
CUDA fast path through the optional ``natten`` package without changing the
state-dict layout.
"""
